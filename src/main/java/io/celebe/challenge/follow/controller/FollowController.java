...
@RestController
@Tag(name = "팔로우 API", description = "팔로우/언팔로우 API")
...
public class FollowController {
    ...
    @ApiResponses({
        ...
        @ApiResponse(responseCode = "400", description = "잘못된 요청 (자기 자신을 팔로우)"),
        @ApiResponse(responseCode = "404", description = "사용자를 찾을 수 없음"),
        @ApiResponse(responseCode = "409", description = "이미 팔로우한 사용자")
    })
    @PostMapping("/follow")
    public ResponseEntity<Void> follow(
        @Parameter(description = "요청자의 식별자", required = true) @RequestHeader("CUSTOM-AUTH-ID") String followerPublicId,
        @Parameter(description = "팔로우할 사용자의 식별자") @PathVariable String followingPublicId
    ) {
    ...
    public ResponseEntity<Void> unfollow(
        @Parameter(description = "요청자의 식별자", required = true) @RequestHeader("CUSTOM-AUTH-ID") String followerPublicId,
        @Parameter(description = "언팔로우할 사용자의 식별자") @PathVariable("publicId") String followingPublicId
    ) {
